...
def _require_native_app(app=None):
    if app is None:
        ...
        app = get_native_app()
        if app is None:
            raise RuntimeError("No current app found. Please provide a Starlette app or ensure that the MplbedMiddleware is installed.")
    return app
...
def setup(
    ...
    *,
    ...
    if do_install_middleware:
        install_middleware(app, prefix=prefix, mplbed_starlette_app=mplbed_starlette_app, mplbed_starlette_app_kwargs=mplbed_starlette_app_kwargs, manage_routing=manage_routing)
