...
class ShowContext:
    target: str
    on_close: str
    prevent_default_navigation: bool = False
...
class FigureManagerWebAggExt(FigureManagerWebAgg):
    ...
    def show(self):
        ...
        html = figure_html_from_id(
            self.num,
            target=show_context.target,
            on_close=show_context.on_close,
            prevent_default_navigation=show_context.prevent_default_navigation,
        )
        add_fig(html)
...
class FigureCollector:
    """Collect HTML generated by ``pyplot.show`` within a context.

    Parameters
    ----------
    target
        Where the collected figures should be created in the page.
    on_close
        Browser behavior to use when a figure WebSocket closes.
    ...
    global_scope
        Whether to use the process-global collection instead of context-local
        collection.
    """
